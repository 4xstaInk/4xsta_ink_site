...
<div 
 @click="showModal = true;headline_data = data"
id="headline-item"
class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl mb-2 cursor-pointer">
        <div class="md:flex">
          <div class="md:flex-shrink-0">
            <img class="h-full w-full object-cover md:w-48" :src="data.urlToImage" alt="A cat">
          </div>
          <div class="p-8">
            <p class="block mt-1 font-bold text-lg text-black">{{data.title}}</p>
            <p class="mt-2 text-gray-500">{{data.description}}</p>
            <hr>
            <p class="flex"><mdi:clock-outline class="mr-2"/> {{
          moment(data.publishedAt).format(
            ...
            <p class="flex"><mdi:face-agent class="mr-2" /> {{data.author}} <span v-if="data.author == ''">No author available</span></p>
